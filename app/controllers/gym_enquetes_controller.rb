class GymEnquetesController < ApplicationController
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to gym_enquetes_path, status: :see_other, notice: "Gym enquete was successfully destroyed." }
  ...
  private
    ...
    def set_gym_enquete
      @gym_enquete = GymEnquete.find(params.expect(:id))
    end

    # Only allow a list of trusted parameters through.
    def gym_enquete_params
      params.expect(gym_enquete: [ :name, :mail, :age, :course_id, :score, :request, :present_id ])
    end
end
